...
class Sam(nn.Module):
    ...
    def forward(
        self, batched_input, multimask_output, image_size, use_dense_embeddings=True
    ):
        if isinstance(batched_input, list):
            outputs = self.forward_test(
                batched_input, multimask_output, use_dense_embeddings
            )
        else:
            outputs = self.forward_train(
                batched_input, multimask_output, image_size, use_dense_embeddings
            )
        return outputs
    ...
    def forward_train(
        ...
        # print("dense embeddings: ", dense_embeddings)
        output_masks, iou_predictions = self.mask_decoder(
            image_embeddings=image_embeddings,
            image_pe=self.prompt_encoder.get_dense_pe(),
            sparse_prompt_embeddings=sparse_embeddings,
            dense_prompt_embeddings=dense_embeddings if use_dense_embeddings else None,
